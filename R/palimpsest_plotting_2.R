...
plot_signatures <- function (input_data = NULL, Title = NA, label = "Full") {
  ...
  for (i in 1:num_of_sigs) {
    ...
    min.y = min(plot_input$freq)

    ### PLot ----------------------------------------------------------------------------------------------------------------

    ggplot(data=plot_input) +
      geom_bar(aes(x=as.factor(plot_input$Context_numerical),y=freq, fill = as.factor(plot_input$Context_numerical)),stat="identity", width = 0.5)+
      ggtitle(plot_title) +
      scale_fill_manual(values = plot_input$Colours, breaks = plot_input$Context_numerical)
    ...
    plot_output= ggplot(data=plot_input) +
      ...
      scale_fill_manual(values = plot_input$Colours, breaks = plot_input$Context_numerical) +
      ylab("") + xlab("") +
      coord_cartesian(xlim = c(1, nrow(plot_input)), ylim = c(min.y,max.y*1.3), clip = 'off') + #  # This keeps the labels from disappearing
    ...
    if (Type == "SBS"){
      if(label == "Full"){
        ...
        plot_output = plot_output +
          ...
          annotate("text",x = 48, y =   min.y-max.y*0.22, label = "Trinucleotide Context", size = 8, fontface = "plain", colour = "black") +
          theme(axis.text.x = element_text(angle = 90, vjust=0.5,  hjust = 0, size = 20, colour = "grey47", family = "mono"),
                axis.text.y = element_text(size = 20),
                axis.ticks.y.right = element_blank()
          ) +
