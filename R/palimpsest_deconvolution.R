...
deconvolution_fit <- function (input_matrices = NULL,
  ...
  for (s in unique(colnames(prop_matrix))) {
    if (doplot == TRUE) {
      ...
      g <- plot_signatures(input_data = Mean_plot_input, Title = paste(s)) + ggtitle(paste(s), subtitle = "Signatures")
      plot(g)
      plotlist[["Signatures"]] <- g
      dev.off()
      if(Type  == "SBS"){
    ...
    fit <- colSums(as.matrix(mat.sigs[1:ncol(mat.sigs)-1])[1,] * input_signatures) / 100
    channels <- names(fit)
    fit <- as.matrix(fit)
    rownames(fit) <- channels

    res <- sum(abs(fit - Mean_plot_input))
    ...
    if (doplot == TRUE) {
      ...
      pdf(file=paste0(resdir..,"/",s,"_",Type,"_fittedprofile.pdf"),width=24,height=7)
      g <- plot_signatures(input_data = fit, Title = paste(s)) + ggtitle(paste(s), subtitle = "Fitted Signatures")
      plot(g)
      plotlist[["Fitted Signatures"]] <- g
      dev.off()

      pdf(file=paste0(resdir..,"/",s,"_",Type,"_profile_residuals.pdf"),width=24,height=7)
      g <- plot_signatures(input_data = Mean_plot_input - fit, Title = paste(s)) + ggtitle(paste(s), subtitle = "Residuals")
      plot(g)
      plotlist[["Residuals"]] <- g
      dev.off()
    }
  ...
  signatures_exp <- list(sig_props = mutSign_props, sig_nums = mutSign_nums, plots = plotlist, residual = res)
